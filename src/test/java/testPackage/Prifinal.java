...
public class Prifinal {

    WebDriver driver;
    WebDriverWait wait;
    private WebElement ShadowHost4; // Moved to instance variable
    private WebElement innerShadowHost; // Added as instance variable
    ...
    public void testPasswordAndLoginButton() {
        try {
            ...
            // Use a temporary variable for the trigger to avoid reassigning outerShadowHost ion-input.ng-untouched.ng-pristine     ion-input.ng-untouched.ng-pristine
            WebElement tempHost = wait.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector(".blur-bg.md.content-ltr.hydrated")));
            ((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView(true);", tempHost);
            tempHost.click();
            Thread.sleep(5000); // Temporary delay (replace with WebDriverWait if possible)
            ...
            // Step 3: Set the inner shadow host as an instance variable
            innerShadowHost = wait.until(webDriver -> {
                WebElement element = (WebElement) ((JavascriptExecutor) driver).executeScript(
                    "return arguments[0].shadowRoot.querySelector('ion-item.item-interactive.item-input')", 
                    outerShadowHost // Effectively final
                );
                return element != null && element.isDisplayed() ? element : null;
            });
            System.out.println("Inner shadow host (ion-item) found: " + innerShadowHost);
            ...
            WebElement passwordInput = wait.until(webDriver -> {
                WebElement element = (WebElement) ((JavascriptExecutor) driver).executeScript(
                    ...
                    innerShadowHost // Now an instance variable, no scoping issue
                );
