...
public class FinalLaunch {
    ...
    WebDriver driver;
    WebDriverWait wait;
    private WebElement ShadowHost4; // Moved to instance variable
    ...
    public void testPasswordAndLoginButton() {
        try {
            ...
            // Use a temporary variable for the trigger to avoid reassigning outerShadowHost
            WebElement tempHost = wait.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector(".native-input.sc-ion-input-md")));
            ...
            // Step 2: Wait for the outer shadow host (ion-content) to be present (effectively final)
            WebElement outerShadowHost = wait.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector(".blur-bg.md.content-ltr.hydrated")));
            ...
            Thread.sleep(2000);
            WebElement innerShadowHost = outerShadowHost.findElement(By.cssSelector("ion-item.item-interactive.item-input"));
            innerShadowHost = wait.until(ExpectedConditions.presenceOfElementLocated(By.cssSelector("ion-item.item-interactive.item-input")));
            // Step 3: Find the ion-input (inner shadow host) inside ion-content's Shadow DOM
           
            WebElement ShadowHost4 = wait.until(webDriver -> {
              WebElement element = (WebElement) ((JavascriptExecutor) driver).executeScript(
                  "return arguments[0].shadowRoot.querySelector('ion-item.item-interactive.item-input')", innerShadowHost); // Now effectively final, no reassignment
            
//            WebElement innerShadowHost = wait.until(webDriver -> {
//                WebElement element = (WebElement) ((JavascriptExecutor) driver).executeScript(
//                    "return arguments[0].shadowRoot.querySelector('ion-item.item-interactive.item-input')", 
//                    outerShadowHost // Now effectively final, no reassignment
//                );
                return element != null && element.isDisplayed() ? element : null;
            });
            System.out.println("Inner shadow host (ion-input) found: " + ShadowHost4);
